...
def get_cov_f_lst(tgt2pred_df, tgt, full_idp_f_lst):
    ...
    if sex_id == 1:
        ...
        cov_f_lst = ["age", "gender", "ethnicity", "BMI", "education", "tdi",
                     "smoking_status", "drinking_status", "Total_IntraCranial_ins2"]
...
def main():
    ...
    cat_cols = ["gender", "ethnicity", "BMI", "education",
    ...
    disease_range = (1, 50)
    all_tgts = tgt2pred_df.Disease_code.tolist()[disease_range[0]-1:disease_range[1]]
    ...
    for tgt in tqdm(all_tgts, desc=f"Disease-loop (autoK)", ncols=100):
        try:
            ...
            subprocess.run(["dx", "upload", imp_path,
                            "--path", f"/Prediction_model/Results/FeatureImportance/{modality_tag}_{tgt}_Top{final_top_k}.csv"], check=True)
            ...
            subprocess.run(["dx", "upload", pred_path,
                            "--path", f"/Prediction_model/Results/Predictions/{modality_tag}_{tgt}_Top{final_top_k}.csv"], check=True)
    ...
    auc_filename = f"{modality_tag}_AUC_summary_autoK_{disease_range[0]}_{disease_range[1]}.csv"
