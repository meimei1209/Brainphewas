...
my_seed = 2024
random.seed(my_seed)
top_k = 30   # Default fallback value if automatic K selection fails
...
def get_iter_predictions(mydf, full_idp_f_lst, cov_f_lst, fold_id, my_params0, my_params, top_k=30):
    ...
    k_records = []  # (K, best_params, mean_auc, se_auc)
...
def main():
    ...
    all_tgts = tgt2pred_df.Disease_code.tolist()
    ...
    for tgt in tqdm(all_tgts, desc=f"Disease-loop (autoK)", ncols=100):
        try:
            ...
            imp_path = os.path.join(dpath, f"Results/FeatureImportance/{tgt}_Top{final_top_k}.csv")
            os.makedirs(os.path.dirname(imp_path), exist_ok=True)
            imp_df.to_csv(imp_path, index=False)
            subprocess.run(["dx", "upload", imp_path,
                            "--path", f"/Prediction_model/Results/FeatureImportance/{tgt}_Top{final_top_k}.csv"], check=True)
            ...
            pred_path = os.path.join(dpath, f"Results/Predictions/{tgt}_Top{final_top_k}.csv")
            os.makedirs(os.path.dirname(pred_path), exist_ok=True)
            pred_df.to_csv(pred_path, index=False)
            subprocess.run(["dx", "upload", pred_path,
                            "--path", f"/Prediction_model/Results/Predictions/{tgt}_Top{final_top_k}.csv"], check=True)
    ...
    auc_filename = f"AUC_summary_autoK.csv"
    auc_path = os.path.join(auc_dir, auc_filename)
    auc_df.to_csv(auc_path, index=False)
    subprocess.run(["dx", "upload", auc_path,
                    "--path", f"/Prediction_model/Results/AUC/{auc_filename}"], check=True)

if __name__ == "__main__":
    main()
# Note:
# All prediction results presented in the manuscript are based solely on the IDP-only model.
# Disease-specific covariate structures vary across conditions; therefore,
# the combined (IDP + covariates) prediction models are provided separately in:
#   Prediction_Dementia.py
#   Prediction_Hyperlipidemia.py
#   Prediction_Cerebrovascular_Disease.py