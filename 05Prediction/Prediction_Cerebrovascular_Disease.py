...
def main():
    ...
    for col in ["ethnicity", "BMI", "smoking_status", "drinking_status","Hypertension", "Atrial_Fibrillation"]:
        if col in cov_df.columns:
    ...
    for tgt in tqdm(all_tgts, desc=f"Disease-loop (autoK)", ncols=100):
        try:
            ...
            subprocess.run(["dx", "upload", imp_path,
                            "--path", f"/Prediction_model/Results/FeatureImportance/Specific_{tgt}_Top{final_top_k}.csv"], check=True)
            ...
            subprocess.run(["dx", "upload", pred_path,
                            "--path", f"/Prediction_model/Results/Predictions/Specific_{tgt}_Top{final_top_k}.csv"], check=True)
    ...
    auc_path = os.path.join(auc_dir, f"AUC_summary_autoK.csv")
    auc_df.to_csv(auc_path, index=False)
    subprocess.run(["dx", "upload", auc_path,
                    "--path", f"/Prediction_model/Results/AUC/Specific_AUC_summary_autoK.csv"], check=True)
